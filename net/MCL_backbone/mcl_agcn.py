...
from net.utils.agcn.graph import Graph
import torch.nn.functional as F
import math
from net.backbone.agcn import TCN_GCN_unit


"""
    多视角融合方式2:MCL和AGCN的结合
"""
...
class MCL_AGCN(nn.Module):
    def __init__(self, in_channels, num_class, device_id, graph_args_agcn,
                 graph_args_stgcn, edge_importance_weighting):
        super().__init__()
        self.view_num = 3

        # load agcn
        self.graph_agcn = Graph(**graph_args_agcn)
        A_agcn = self.graph_agcn.A
        self.data_bn = nn.BatchNorm1d(2 * in_channels * 25)

        self.agcn = nn.ModuleList((
            TCN_GCN_unit(3, 64, A_agcn, residual=False),
            TCN_GCN_unit(64, 64, A_agcn),
            TCN_GCN_unit(64, 64, A_agcn),
            TCN_GCN_unit(64, 64, A_agcn),
            TCN_GCN_unit(64, 128, A_agcn, stride=2),
            TCN_GCN_unit(128, 128, A_agcn),
            TCN_GCN_unit(128, 128, A_agcn),
            TCN_GCN_unit(128, 256, A_agcn, stride=2),
            TCN_GCN_unit(256, 256, A_agcn),
            TCN_GCN_unit(256, 256, A_agcn)
        ))
    ...
    def forward(self, x1, x2, x3):
        ...
        for view_index in range(self.view_num):
            ...
            for agcn_layer in self.agcn:
                x[view_index] = agcn_layer(x[view_index])
        # prediction
...
if __name__ == "__main__":
    model = MCL_AGCN(in_channels=3,num_class=60,device_id=0,edge_importance_weighting=True,
